...
export function createBuses(bedUrl: string): Buses {
  ...
  voiceComp.connect(level)
  archiveHp.connect(level)
  ;(voiceComp as unknown as { busInput: Tone.ToneAudioNode }).busInput = voiceComp
  return {
    voice: voiceComp as unknown as Tone.Gain,
    archive: archiveHp as unknown as Tone.Gain,
    bed,
    foley,
    bedPlayer,
    level,
...
export function duck(buses: Buses) {
  ...
  const reading = buses.level.getValue()
  const target = Tone.dbToGain(typeof reading === 'number' && reading > 0.01 ? -36 : -30)
