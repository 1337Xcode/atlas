...
export const usePhases = create<PhaseState>((set) => ({
  ...
  setEvent: (eventId) => set({ eventId, phase: 'hub', stopIndex: -1 }),
  resolveMark: (id) =>
    set((s) => (s.resolvedMarks.includes(id) ? s : { resolvedMarks: [...s.resolvedMarks, id] })),
  toggleProvenance: () => set((s) => ({ provenanceVisible: !s.provenanceVisible })),
  setStop: (stopIndex) => set({ stopIndex }),
}))
