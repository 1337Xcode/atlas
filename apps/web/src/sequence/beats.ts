...
export function initialValues(sc: Scenario): Record<string, number> {
  ...
  for (const [id, p] of Object.entries(sc.pages)) {
    init['plane.' + id + '.o'] = p.hub ? 1 : 0
    init['plane.' + id + '.slide'] = p.slideFrom !== undefined ? 0 : 1
  }
  return init
}
...
export function buildTracks(sc: Scenario): Tracks {
  const tracks: Tracks = {}
  const add = (name: string, t: number, v: number, dur: number | undefined) => {
    ;(tracks[name] ??= []).push({ t, v, dur: Math.max(dur ?? 1.5, 0.05) })
  }
  const grab = (
    b: Beat,
    ...
    for (const k of keys) {
      const v = obj[k]
      if (v !== undefined) add(prefix + k, obj.at ?? b.t, v, obj.dur ?? ddur)
    }
  }
  for (const b of sc.beats) {
    grab(b, b.camera, ['x', 'y', 'z'], 'camera.', 2)
    grab(b, b.focus, ['x', 'y', 'radius'], 'focus.', 1.5)
    grab(b, b.post, ['grain', 'vignette', 'aberration'], 'post.', 1.5)
    ...
    for (const [id, pl] of Object.entries(b.planes ?? {})) {
      if (pl.o !== undefined) add('plane.' + id + '.o', pl.at ?? b.t, pl.o, pl.dur)
      if (pl.slide !== undefined) add('plane.' + id + '.slide', pl.at ?? b.t, pl.slide, pl.dur)
    }
  }
  return tracks
}
...
export function valueAt(
  ...
  for (const k of tracks[name] ?? []) {
    if (k.t > t) break
    let from = v
    if (base && !usedBase && k.t >= base.t) {
      from = base.v
      usedBase = true
    }
    const p = Math.min(1, Math.max(0, (t - k.t) / k.dur))
    v = from + (k.v - from) * ease(p)
  }
  if (base && !usedBase && t >= base.t) return base.v
  return v
}

export function phaseAt(sc: Scenario, t: number): Phase {
  let ph: Phase = 'hub'
  for (const b of sc.beats) {
    if (b.t <= t) ph = b.phase
    else break
...
export function drift(t: number) {
  return { x: 0.004 * Math.sin(t * 0.5), y: 0.004 * Math.cos(t * 0.33) }
}
